...
reof_dat <- reof_raster %>%
  ...
  select(-ID) %>%
  rename(name = `sites$SWSN_Site`)
reof_raster_3 <- brick('output/reofs_3.tif')  %>%
  `names<-`(., paste0('reof', 1:nlayers(.)))
reof_dat_3 <- reof_raster_3 %>%
  raster::extract(sites, df = TRUE) %>%
  cbind(sites$SWSN_Site, .) %>%
  select(-ID) %>%
  rename(name = `sites$SWSN_Site`)
...
reof_raster %>%
  ...
  scale_fill_distiller(palette = 'BrBG', limits = c(-1, 1)) +
  geom_sf(data = states, fill = NA, color = 'black') +
  geom_sf(data = filter(sites, P1room > 0)) +
  facet_wrap(~layer) +
  coord_sf(datum = NA) +
  theme_void()
...
reof_raster %>%
  ...
  geom_sf(data = states, fill = NA, color = 'black') +
  geom_sf(data = filter(sites, P3room > 0)) +
  coord_sf(datum = NA) +
  theme_void()
```
...
swsn <-sites %>%
  group_by(cell) %>%
  sample_n(1) %>%
  left_join(coarse_jsd, ., by = c('name' ='cell'))

swsn_reof <- swsn  %N>%
  mutate(reof1 = raster::extract(reof_raster, st_coordinates(geometry))[,1],
         reof2 = raster::extract(reof_raster, st_coordinates(geometry))[,2],
         reof3 = raster::extract(reof_raster, st_coordinates(geometry))[,3],
         reof4 = raster::extract(reof_raster, st_coordinates(geometry))[,4],
         reof5 = raster::extract(reof_raster, st_coordinates(geometry))[,5],
         reof6 = raster::extract(reof_raster, st_coordinates(geometry))[,6]) %E>%
  #mutate(elev = raster::extract(elev, st_coordinates(geometry))) %E>%
#  mutate(ws = raster::extract(ws, st_coordinates(geometry))) %E>%
  mutate(JSD = if_else(JSD > .999, .999, JSD)) %>%
  #filter(between(JSD, .001, .999)) %>%
  ...
  #filter(from < to) %>%
  convert(to_undirected) %E>%
    mutate(dist_geo = geosphere::distGeo(st_coordinates(.N()$geometry[from]), st_coordinates(.N()$geometry[to])) / 1000) %E>%
...
#left_join(eof_dat) %>%
 ...
 #   ws = abs(.N()$ws[from] - .N()$ws[to]),
#    ws_mul = .N()$ws[from] * .N()$ws[to],
    rooms = case_when(
      time == 1200 ~ .N()$P1room[from] * .N()$P1room[to],
...
dat <- swsn_reof %E>%
  as_tibble %>%
  mutate(from = as.factor(from), to = as.factor(to), time_num = time, time = as.factor(time)) %>%
  mutate(tie = JSD >= .001) %>%
  filter(tie == TRUE)
...
ggplot(dat, aes(log(ws_mul))) +
  geom_histogram() + facet_wrap(~time)
ggplot(dat, aes(log(elev+1))) +
  geom_histogram() + facet_wrap(~time)
ggplot(dat, aes((ws))) +
  geom_histogram() + facet_wrap(~time)
```
