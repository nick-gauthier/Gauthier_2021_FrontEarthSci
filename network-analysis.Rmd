...
env_rasters <- brick(c(elev, ws, reofs))
```
...
models <- c(
 "s(distance, bs = 'cr', k = 6) + time_fact",
      ...
      s(reof6, by = time_fact, bs = 'cs', k = 6) + time_fact",
        "s(distance, bs = 'cr', k = 6) +
          s(reof1, by = time_fact, bs = 'cs', k = 6) + time_fact",
        "s(distance, bs = 'cr', k = 6) +
          s(reof2, by = time_fact, bs = 'cs', k = 6) + time_fact",
        "s(distance, bs = 'cr', k = 6) +
       s(reof3, by = time_fact, bs = 'cs', k = 6) + time_fact",
   "s(distance, bs = 'cr', k = 6) +
       s(reof4, by = time_fact, bs = 'cs', k = 6) + time_fact",
   "s(distance, bs = 'cr', k = 6) +
       s(reof5, by = time_fact, bs = 'cs', k = 6) + time_fact",
   "s(distance, bs = 'cr', k = 6) +
       s(reof6, by = time_fact, bs = 'cs', k = 6) + time_fact"
   ) %>%
  paste('qlogis(JSD) ~', .) %>%
...
```{r}
m1 <- gamm(qlogis(JSD) ~ s(distance, bs = 'cr', k = 6),
      method = 'REML', 
      random = list(time_fact = ~1),
      family = gaussian(),
      correlation = corMLPE(form = ~from + to|time_fact),
      data = dat)
m15 <- gamm(qlogis(JSD) ~ s(distance, bs = 'cr', k = 6) +time_fact,
      method = 'REML', 
      family = gaussian(),
      correlation = corMLPE(form = ~from + to|time_fact),
      data = dat)
...
plt_dat <- swsn %E>% 
  ...
  mutate(JSD = if_else(JSD > .999, .999, JSD),
         JSD = qlogis(JSD),
         pred_mod = (predict(m2$lme)[test]),
         pred_null = ((predict(m1$lme))[test]),
         res_null = JSD - pred_null, #residuals(models$lme[[8]], type = 'normalized')[test],
         res_mod = JSD - pred_null) %>% #residuals(models$lme[[1]], type = 'normalized')[test]) %>%
  filter(time == 1200)
```  
...
ggraph(x = x, y = y) +
  ...
  scale_edge_color_viridis(option = 'magma', guide = 'none') +
   labs(subtitle = 'A') +
  theme_void() +
  coord_sf(datum = NA)
...
ggraph(x = x, y = y) +
  ...
  scale_edge_alpha(guide = 'none') +
  scale_edge_color_distiller(palette = 'RdYlBu', guide = 'none', name = 'JSD') + 
   labs(subtitle = '') +
  theme_void() +
  coord_sf(datum = NA)
...
```{r}
plt_dat %>%
  select(JSD,pred_mod:res_mod) %>%
  mutate(JSD = qlogis(JSD),
         pred_mod = qlogis(pred_mod),
         test = JSD - pred_mod)
  mutate(test = plogis(qlogis(pred_mod) + res_mod)) %>%
arrange(test) %>%
ggraph(x = x, y = y) +
...
```{r}
m1 <- dat %>%
  bam(JSD ~ s(distance, bs = 'cr', k = 6) +
        te(reof1, time, k = c(6, 4), fx = c(F,T)) +
         te(reof2, time, k = c(6, 4), fx = c(F,T)) +
         te(reof3, time, k = c(6, 4), fx = c(F,T)) +
         te(reof4, time, k = c(6, 4), fx = c(F,T)) +
         te(reof5, time, k = c(6, 4), fx = c(F,T)) +
         te(reof6, time, k = c(6, 4), fx = c(F,T)) +
        s(time, from, bs = 'fs', k = 4), data = ., method = 'fREML', discrete = TRUE, family = betar, nthreads = 2)
...
m1 <- dat %>%
  bam(JSD ~ s(distance, bs = 'cr', k = 6) +
        ...
        s(reof5, bs = 'cr',  k = 6) +
        s(reof6, bs = 'cr',  k = 6) +
         s(time, from, bs = 'fs', k = 4),
      data = ., method = 'fREML', discrete = TRUE, select = TRUE, family = betar, nthreads = 2)

m2 <- dat %>%
  filter(tie == TRUE) %>%
    mutate(from = as.factor(from), to = as.factor(to)) %>%
  bam(JSD ~  s(time, bs = 'cr', k = 4, m = 2) + s(time, by = from, bs = 'cs', k = 4, m = 1) + s(from, bs = 're') + s(distance, bs = 'cr', k = 5), data = ., method = 'fREML', discrete = TRUE, family = betar, nthreads = 2)

AIC(m1,m2)
plot(m1, trans = plogis, scale = 0)
summary(m1)
summary(m2)
```
...
swsn_reof %E>% 
  mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  filter(JSD > 0) %>%
  mutate(pred = predict(m1, newdata = as_tibble(.), type = 'response')) %>%
  arrange(pred) %>%
ggraph('manual', node.positions = as.data.frame(pts_jsd)) +
...
swsn_reof %E>% 
  ...
  mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  filter(JSD > 0) %>%
  mutate(pred = predict(m4, newdata = as_tibble(.), type = 'response')) %>%
  arrange(pred) %>%
ggraph('manual', node.positions = as.data.frame(pts_jsd)) +
  geom_edge_fan(aes(alpha = pred, color = pred)) +
  geom_sf(data = states, fill = NA, color = 'black') +
...
swsn_reof %E>% 
  ...
  mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  filter(JSD > 0) %>%
  mutate(pred = JSD - predict(m3, newdata = as_tibble(.), type = 'response')) %>%
  arrange(pred) %>%
ggraph('manual', node.positions = as.data.frame(pts_jsd)) +
  geom_edge_fan(aes(alpha = pred, color = pred)) +
  geom_sf(data = states, fill = NA, color = 'black') +
