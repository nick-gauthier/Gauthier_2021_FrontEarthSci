...
models <- c(
  "s(time, by = from, bs = 'cs', k = 4, m = 1) + 
  s(from, bs = 're')",
  "s(log(rooms), bs = 'cr', k = 6) + s(distance, bs = 'cr', k = 6) +
  s(time, by = from, bs = 'cs', k = 4, m = 1) + s(from, bs = 're')",
"s(distance, bs = 'cr', k = 6) + s(log(rooms), bs = 'cr', k = 6) +
...
gam.check(dist_models$model[[3]])
plot(models$model[[2]], scheme = 1)
```
...
models <- c(
 "s(distance, bs = 'cr', k = 6)",
       "s(distance, bs = 'cr', k = 6) +
          s(reof1, by = time_fact, bs = 'cr', k = 6) +
          s(reof2, by = time_fact, bs = 'cr', k = 6) +
       s(reof3, by = time_fact, bs = 'cr', k = 6) +
       s(reof4,  by = time_fact, bs = 'cr', k = 6) +
      s(reof5, by = time_fact, bs = 'cr', k = 6) +
      s(reof6, by = time_fact, bs = 'cr', k = 6)",
        "s(distance, bs = 'cr', k = 6) +
          s(reof1, by = time_fact, bs = 'cr', k = 6)",
        "s(distance, bs = 'cr', k = 6) +
          s(reof2, by = time_fact, bs = 'cr', k = 6)",
...
plot(models$gam[[1]], pages = 1)
plot(models$gam[[1]], trans = plogis)
```
...
geom_line(aes(color = reof), size = 1.2) +
  ...
  theme_bw() +
  labs(title = 'Drought patterns and the intensity of social interaction', subtitle = "Predicted information flow at a 10 days' travel", x = 'Absolute Difference', y = 'Similarity')
...
plt_dat <- swsn %E>% 
  ...
  filter(time == 1250, 
         JSD > 0.001) %>%
  mutate(JSD = if_else(JSD > .999, .999, JSD),
         pred_mod = plogis(predict(models$gam[[1]], newdata = as_tibble(.), type = 'response')),
         pred_null = plogis(predict(models$gam[[8]], newdata = as_tibble(.), type = 'response')),
         res_null = (JSD) - pred_null,
         res_mod = (JSD) - pred_mod)
```  

```{r}
a <- plt_dat %>%
arrange(JSD) %N>%
ggraph(layout = 'auto', x = 'x', y = 'y') +
...
ggraph('manual',x = x, y = y) +
  ...
  scale_edge_alpha(guide = 'none', limits = c(0,1)) +
    #scale_edge_color_viridis(option = 'magma', guide = 'none', direction = -1) +
    scale_edge_color_distiller(palette = 'RdYlBu', guide = 'none', name = 'JSD', limits = c(-1,1)) +
  labs(subtitle = 'D') +
  theme_void() +
  coord_sf(datum = NA)
