...
fit_gam_JSD <- function(x){
   bam(x, method = 'fREML',
      ...
      data = mutate(dat, from = as.factor(from)))
}
plan(multicore, workers = 2)

dist_models <- c(
  "s(from, bs = 're') + as.factor(time)",
"s(from, bs = 're') + s(dist_geo, bs = 'cr', k = 6) + as.factor(time)",
"s(from, bs = 're') + s(distance, bs = 'cr', k = 6) + as.factor(time)"
...
dist_models
summary(dist_models$model[[1]])
gam.check(dist_models$model[[1]])
plot(dist_models$model[[1]], trans = plogis)
```
...
m2 <- dat %>%
  ...
  bam(JSD ~ s(from, bs = 're') + 
        as.factor(time), 
      data = ., discrete = TRUE, family = betar, select = TRUE)
m3 <- dat %>%
  mutate(from = as.factor(from)) %>%
  bam(JSD ~ s(from, bs = 're') + 
        s(time, k = 5) + macro_same, 
      data = ., discrete = TRUE, family = betar, select = TRUE)

m1 <- dat %>%
  mutate(from = as.factor(from)) %>%
  bam(JSD ~ s(from, bs = 're') + 
         s(distance, k = 6, bs ='cr') +
...
models <- c(
"s(dist_geo, bs = 'cr', k = 6)",
#"s(distance, bs = 'cr', k = 6) + s(log(ws_mul), bs = 'cs', k = 6)",
      "s(dist_geo, bs = 'cr', k = 6) + 
...
```{r}
concurvity(geo_models2$gam[[1]])
concurvity(geo_models2$gam[[2]])
```

```{r}
evaluate_smooth(models$gam[[1]], 's(dist_geo)')  %>% draw()
evaluate_smooth(models$gam[[1]], 's(dist_geo)') %>% 
...
models$gam[[1]] %>% getViz %>% sm(1) %>%  plot(trans = plogis) +geom_vline(xintercept = c(8, 80), linetype = 2)+ l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) 
ggsave('figures/distance_function.pdf', width = 6, height = 4)
```

```{r}
draw(models$gam[[1]], scales = 'fixed')
...
plt_dat <- swsn %E>% 
  filter(time == 1200, 
         JSD > 0.001) %>%
  mutate(JSD = if_else(JSD > .999, .999, JSD),
         pred_mod = (predict(dist_models$model[[1]], newdata = as_tibble(.), type = 'response')),
         pred_null = (predict(dist_models$model[[3]], newdata = as_tibble(.), type = 'response')),
          #pred_null = (predict(m3, newdata = as_tibble(.), type = 'response')),
         #pred_mod = (predict(m4, newdata = as_tibble(.), type = 'response')),
...
m1 <- dat %>%
  ...
  bam(JSD ~ s(from_time, bs = 're') + s(to_time, bs = 're') + s(dist_geo, bs ='cr', k = 6) +time, data = ., discrete = TRUE, family = betar)
m2 <- dat %>%
  filter(tie == TRUE) %>%
    mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  gam(JSD ~ s(from_time, bs = 're') + s(dist_geo, bs ='cr', k = 6) + time , data = ., discrete = TRUE, family = betar)

m3 <- dat %>%
  mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  bam(JSD ~ s(from_time, bs = 're') + s(dist_geo, k = 6, bs ='cr') +
         te(reof1, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof2, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof3, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof4, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof5, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof6, time_num, k = c(6, 4), fx = c(F,T)) + time, data = .,discrete = TRUE, family = betar)

m4 <- dat %>%
  filter(tie == TRUE) %>%
  mutate(from_time = interaction(from, time),
         to_time = interaction(to, time)) %>%
  bam(JSD ~ s(from_time, bs = 're') + s(to_time, bs = 're') + s(dist_geo, k = 6, bs ='cr') +
         te(reof1, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof2, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof3, time_num, k = c(6, 4), fx = c(F,T)) +
         te(reof4, time_num, k = c(6, 4), fx = c(F,T)) +
