...
So from this plot, we can tell that PCs 1, 2, 3, and 5 are of the right sign (high pc amplirude == high positive SPEI), and PCs 4 and 6 need to be reversed.
```{r}
obs_reof_amp %>%
  group_by(PC) %>%
...
recon_amplitudes %>%
  ...
  mutate(recons = if_else(PC %in% c('REOF4', 'REOF6'), recons * -1, recons),
         amp_smooth = if_else(PC %in% c('REOF4', 'REOF6'), amp_smooth * -1, amp_smooth)) %>%
