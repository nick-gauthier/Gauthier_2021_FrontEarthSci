...
the overlaps mean these are effective multplets. the real eof can be some linear combination of these. This won't in practice impact our successive results, as long as we don't truncate within these multplets, but only between them.
...
The colors here are eigenvectors weighted by the sqare root of the associated eigenvalue, so these loadings represent the covariance between each grid cell and each amplitdue (principal component). A key assumption here is that the REOFs calculated from both the observations and reconstructions correspond to the same physical phenomena. That way we can just use the observed patterns as the high resolution patterns, and infer their temporal evolution from the reconstructions. This means we don't have to downscale the reconstructed reofs to use them, as otherwise we'd have to deal with issues of spatial represntativeness and non-overlap. Now we can be ensured that these are the same signals.
...
recon_reof_amp <- recon_reof$amplitude %>%
  ...
  mutate(year = parse_number(year)) %>%
  gather(PC, amplitude, -year) %>%
  mutate(PC = if_else(PC == 'EOF2', 'EOF4', if_else(PC == 'EOF4', 'EOF2', PC)))

amplitude_modern <- obs_reof_amp %>%
...
t1 <- bam(amplitude_obs ~ s(amplitude_recon, by = PC) + PC,
      ...
      AR.start = new_pc,
      data = mutate(amplitude_modern, new_pc = PC != lag(PC)))
...
past_amplitudes <- recon_reof$amplitude %>%
  ...
  rename(amplitude_recon = recon) %>%
  group_by(reof) %>%
  nest(.key = 'recons')
past_amplitudes <- recon_reof_amp %>%
...
recon_amplitudes <- amplitude_modern %>%
  ...
  mutate(predictions = purrr::map2(mod, recons, 
         ...
         predictions = purrr::map(predictions, 
                                  ~tibble(amplitude = .,
                                          amp_smooth = zoo::rollmean(amplitude, k = 15,fill = NA)))) %>%
...
ggplot(filter(recon_amplitudes, between(year, 1200, 1450)), aes(year, amplitude, group = PC)) +
  geom_line(aes(color = PC), alpha = .7) +
...
ggplot(recon_amplitudes, aes(year, amplitude, group = PC)) +
  geom_line(aes(color = PC), alpha = .7) +
  #geom_line(aes(y = amp_smooth)) +
  scale_color_brewer(palette = 'Spectral') +
  facet_wrap(~PC) +
...
recon_amplitudes %>%
  ...
  ggplot(aes(factor(period), abs(amplitude))) +
  geom_boxplot() +
...
recon_amplitudes %>%
  ...
  group_by(PC, time) %>% 
  summarise(amplitude = mean(abs(amplitude))) %>%
  mutate(time = as.factor(time)) %>%
  write_csv('output/amplitudes.csv')

recon_amplitudes %>%
  filter(between(period, 1200, 1400)) %>%
  ggplot(aes(abs(amplitude), as.factor(period),fill = ..x.., height = ..density..)) +
...
```
```{r}
rasterVis::levelplot(brick(c(reof_raster[[1]] * .28, reof_raster[[1]] * .39)))
```
