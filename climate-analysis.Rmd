...
country <- maps::map('usa',fill = TRUE, plot = FALSE) %>%
  st_as_sf
...
ggplot() + 
  ...
  geom_sf(data = bbox_swsn, fill = NA) +
  theme_bw()
```
...
# calculate average JJA SPEI
spei_obs <- brick('data/spei12_8_PRISM.nc') %>%
  crop(bbox_wus) %>% # crop to the study area bounding box
...
ggplot(aes(x = PC, y = percent * 100)) +
  ...
  theme_bw() + 
  guides(color = F) + 
  scale_x_continuous(breaks = c(4, 8, 12)) #+ scale_y_continuous(trans = 'log')# + stat_smooth(aes(weight = weights), method = 'lm') 
...
```{r plot_robs_eof, echo = FALSE, fig.width = 6, fig.height = 8, fig.cap = 'Observed drought REOFs'}
as.data.frame(reof_raster, xy = TRUE, na.rm = TRUE) %>%
  mutate(REOF3 = REOF3 * 1, REOF6 = REOF6 * -1) %>%
  gather(eof, value, 3:(n_modes + 2)) %>%
  #mutate(value = value * -1) %>%
  ggplot() +
  geom_raster(aes(x, y, fill = value)) +
  #scale_fill_distiller(palette = 'BrBG', name = 'Correlation', direction = -1, limits = c(-1,1)) +
...
ggsave('figures/reof_observed.png', height = 8, width = 6)
```
...
ggplot() +
  ...
  geom_raster(aes(x, y, fill = value)) +
  scale_fill_distiller(palette = 'BrBG', direction = 1, limits = c(-.81, .81)) +
  theme_void() + 
    geom_sf(data = states, color = 'black', fill = NA) +
  coord_sf(datum = NA) +
  theme(legend.position = "bottom")
...
ggplot(eofs_recon, aes(x, y, fill = value)) +
  facet_wrap(~PC) +
  geom_raster() +
  scale_fill_distiller(palette = 'BrBG', direction = 1, limits = c(-.74, .74)) +
  theme_void() + 
  coord_quickmap()
```
...
as.data.frame(spei_recon[[1]], xy = TRUE, na.rm = TRUE) %>% cbind(recon_reof$REOF) %>%
  select(-3) %>% 
    mutate(EOF1 = EOF1 * -1, EOF3 = EOF3 * -1,EOF4 = EOF4 * -1,EOF6 = EOF6 * -1) %>%
  gather(eof, value, 3:(n_modes+2)) %>%
  ggplot() +
  geom_raster(aes(x,y,fill = value)) +
...
```{r amplitudes, eval = FALSE, include = FALSE}
...
So from this plot, we can tell that all but PC6 is of the right sign (high pc amplirude == high positive SPEI)
```{r}
obs_reof_amp %>%
  group_by(PC) %>%
...
reof_cor %>% 
  ...
  theme_bw()
ggsave('figures/sst_correlation.png', width = 7.5)
```
...
ggsave('figures/spei_reconstruction.pdf', width = 10, height = 4)
```
