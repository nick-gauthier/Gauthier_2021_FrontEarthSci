...
ggsave('figures/scree.pdf', height = 4, width = 5)
```
...
obs_reof <- spei_obs %>% 
  ...
  eof(n_modes, scale. = FALSE) # don't rescale (ie we use the covariance matrix, because spei is already normalized and rescaled)
...
```{r plot_robs_eof, echo = FALSE, fig.width = 6, fig.height = 7, fig.cap = 'Observed drought REOFs'}
...
```{r plot_recon_amplitudes, fig.width = 6, fig.height = 5, echo = FALSE, fig.fullwidth=TRUE}
recon_amplitudes %>%
...
ggsave('figures/spei_reconstruction.pdf', width = 6, height = 5)
```
