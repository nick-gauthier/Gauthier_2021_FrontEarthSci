...
swsn %N>%
  ...
  coord_sf(datum = NA) +
  theme_void()  +
  theme(legend.position = c(1, 0), legend.justification = c(2.75,-.5))
ggsave('figures/site_distribution.pdf', width = 6, height = 4)
```

```{r fig.fullwidth = TRUE, fig.width = 6, fig.height = 5}
swsn %E>% 
    filter(JSD > .001) %>%
    filter(from < to) %>%
...
ggsave('figures/JSD_network2.png', width = 6, height = 5)
```
