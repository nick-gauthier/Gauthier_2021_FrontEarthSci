...
qplot(slope, tobler_adjusted(abs(slope)), geom = 'line' ) +
  ...
  labs(x = 'Slope (rise over run)', y = 'Walking speed (m/s)') +
  theme_classic()
```
...
```{r least-cost-distances, cache = TRUE, dependson=c('conductance')}
distances <- sites %>%
  st_coordinates %>%
  ...
  as_tibble %>%
  mutate(from = sites$SWSN_Site) %>%
  gather(to, distance, -from) # wide to long
```

```{r}
write_csv(distances, 'output/distanes.csv')
```
...
nn_mat <- dist_mat %>% 
  ...
  group_by(from) %>% 
  top_n(-12, distance) %>% # retain the top 15 smallest (i.e. shortest) edges
