...
```{r nearest_neighbor}
nn_mat <- swsn %N>%
  select(-site, -geometry) %E>%
  mutate(from_patch = .N()$name[from],
         to_patch = .N()$name[to]) %>%
  left_join(distances) %>%
  morph(to_split, time, split_by = 'edges') %>% # split into subgraphs
  group_by(from) %>% 
  top_n(-10, distance) %>% # retain the top 15 smallest (i.e. shortest) edges
...
ggraph(nn_mat, 'manual', x =x, y=y) +
  geom_edge_link(alpha = .1) +
  facet_edges(~time, nrow = 1) +
  theme_void() +
  coord_equal() +
  ggtitle('Nearest neighbor network', 'k = 10')
```
...
ggraph(time_mat, 'manual', x =x, y = y) +
  geom_edge_link(alpha = .1) +
  theme_void() +
  facet_edges(~time) +
  coord_equal()
...
path_dat <- swsn_reof %>%   filter(distance <= 12) %E>% # neighbor matrix edgelist
  as_tibble %>% 
...
nn_mat %E>%
  ...
  mutate(len = st_length(geometry),
         len = as.numeric(len) / 1000) %>%
  ggplot(aes(as.numeric(len), distance)) +
  geom_point() +
...
nn_mat %E>%
  ...
  as_tibble %>%
  ggplot() +
    geom_raster(data = as.data.frame(raster(speed_c), xy = T, na.rm = T), aes(x, y, fill = layer)) +
    geom_sf(alpha = .5) +
