...
So from this plot, we can tell that all but PC6 is of the right sign (high pc amplitude == high positive SPEI). so in the next plot, we multiply pc6 by -1 
```{r, echo = FALSE}
reof_obs$amps %>%
  group_by(PC) %>%
...
reof_recon$amps %>%
  ...
  ggplot(aes(factor(period), abs(reof_amp))) +
  geom_boxplot() +
...
reof_recon$amps %>%
 ...
 # mutate(PC = paste0('reof',parse_number(PC))) %>%
  ...
  group_by(PC, time) %>% 
  summarise(amplitude = mean(abs(reof_amp))) %>%
...
```{r}
m1$gam %>% getViz %>% sm(1) %>%  plot(trans = plogis) +geom_vline(xintercept = c(12, 100), linetype = 2)+  l_ciPoly(mul = 5) + l_fitLine() + scale_y_continuous(limits = c(0,1)) + labs(x = 'Distance (hours)', y = 'Information flow')
