...
Now map this GAMM formulat to a list of model specifications, extracting the AIC from each model fit and ordering the result accordingly. Not that the Jensen-Shannon values are logit transformed before modeling (using qlogis()) to normalize the response data.^[Note the Jensen Inequality here, namely E(logit(y)) >= logit(E(y)). This means we should interpret specific predictions and confidence intervals from this model with caution, and instead only focus on interpretaiton of the brod functional forms. It would be preferable to run the model as a beta regression without logit-transformation, but it is not currently possible to fit the corMLPE correlation structure with a beta family. Preliminary tests with the data suggested that the pairwise correlation structure was a larger source of bias than transforming the response variabble, so this was a reasonable tradeoff.]
 
```{r, echo= FALSE, fig.margin = TRUE, fig.cap = 'Distribution of network tie strengths over time.'}
ggplot(dat, aes(JSD)) +
  geom_histogram(bins = 30) + facet_wrap(~time)
```

```{r, echo = FALSE, fig.margin = TRUE, fig.cap='Same as above, only with logit-transformed tie strengths.'}
...
Examining the resulting models shows that the model including the REOFs as covariates results in a lower AIC.
```{r, echo = FALSE}
models %>%
