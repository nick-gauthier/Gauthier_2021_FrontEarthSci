...
```{r}
obs_pca <- spei_obs %>%
  area_weight %>% # reweight for latitudinal distortions
...
get_EOFs <- function(pc_object, eigs, rast_template, n_modes){
  ...
  reofs <- unclass(varim$loadings) %>%
    ...
    pivot_longer(cols = -column, names_to = 'PC', values_to = 'reof') %>%
  ...
  eof_amps <- pc_object$x %>%
    ...
    pivot_longer(-year, names_to = 'PC', values_to = 'eof_amp') %>%
  ...
  reof_amps <- pc_object$x %>%
    ...
    scale %>%
    `%*%`(rot_mat) %>%
    as_tibble(rownames = 'year', .name_repair = ~paste0('PC', 1:n_modes)) %>%
    pivot_longer(cols = -year, names_to = 'PC', values_to = 'reof_amp') %>%
    mutate(year = as.numeric(str_sub(year, 2)),
           PC = as.factor(PC))
...
reof_raster <- reof_obs$EOFs %>%
  select(-eof) %>%
  pivot_wider(names_from = PC, values_from = reof) %>%
  rasterFromXYZ %>%
...
Next we get the apportioned ceramic ware counts per site. These files are supplied by the SWSN project as a result from their internal apportioning scheme. Start by loading these preprocessed matrices, one for each time step, and combining them into a single tibble.
...
The best performing model includes distance and REOFs as predictors and had an AIC of `r models[[1,6]]`. In second is the model with distance alone, with an AIC of `r models[[2,6]]`. The REOF + distance model has an adjusted R-squared of `r round(summary(models[[1,5]])$r.sq, 3)`, and the distance only model has an adjusted R-squared of `r round(summary(models[[2,5]])$r.sq, 3)`. 
