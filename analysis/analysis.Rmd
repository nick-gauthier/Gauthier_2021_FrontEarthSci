...
get_EOFs <- function(pc_object, eigs, rast_template, n_modes){
  ...
  list(EOFs = reofs, amps = left_join(eof_amps, reof_amps), rotater = rot_mat)
}
```
...
#do the eofs match?
reof_recon$amps %>% 
  inner_join(reof_obs$amps, by = c('year', 'PC'), 
            suffix = c('_recon', '_obs')) %>%
  ggplot(aes(eof_amp_recon, eof_amp_obs)) +
...
```{r}
## so now the reofs for the recons don't match, do something like this:
...
reof_recon$amps %>%
  group_by(PC) %>%
  mutate(amplitude = if_else(PC == 'PC2', amplitude * -1, amplitude)) %>%
  filter(year >= 1896) %>%
  mutate(amp_smooth = zoo::rollmeanr(amplitude, k = 10, fill = NA)) %>%
ggplot(aes(year, amplitude, color = PC)) +
...
### so here's where we leave off -- why don't these match well anymore? # perhaps use the rotation matrix from the observations to rotate the recons?
```
