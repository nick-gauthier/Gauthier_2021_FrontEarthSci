...
amplitudes %>%
  ...
  unnest %>%
  ggplot(aes(year, amplitude, group = eof)) +
  geom_line(aes(color = eof), alpha = .7) +
  geom_line(aes(y = zoo::rollmean(amplitude, k = 50, na.pad = T))) +
...
spei_coarse <- brick('~/Downloads/da_hydro_JunAug_r.1-2000_d.05-Jan-2018.nc') %>%
  .[[1100:1499]] %>%
  rotate %>%
