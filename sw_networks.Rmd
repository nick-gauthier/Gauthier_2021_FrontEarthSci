...
geo_models
summary(geo_models$gam[[2]])
plot(geo_models$gam[[1]])
```

```{r}
swsn_reof %E>% 
  filter(similarity > 0) %>%
  mutate(pred = predict(geo_models$gam[[4]], newdata = as_tibble(.), type = 'response')) %>%
    mutate(res = qlogis(similarity) - (pred)) %>%
  as_tibble %>%
  ggplot(aes(reof4, res, group = time, color = as.factor(time))) +
  geom_smooth()
```
...
plt_dat <- swsn_reof %E>% 
  filter(time == 1200, 
         similarity > 0) %>%
  mutate(similarity = if_else(similarity > .999, .999, similarity),
        # pred_mod = plogis(predict(geo_models$gam[[2]], newdata = as_tibble(.), type = 'response')),
         pred_null = plogis(predict(geo_models$gam[[3]], newdata = as_tibble(.), type = 'response')),
         res = similarity - pred_null) %>%
  select(similarity, time, pred_null:res)
