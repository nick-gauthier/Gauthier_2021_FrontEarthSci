...
ggplot(aes(long, lat)) +
  ...
  geom_path(aes(group = group), alpha = .2, color = 'lightgrey') +
  scale_fill_viridis_c(direction = -1) +
  facet_wrap(~time, nrow = 1) +
  ggtitle('Nearest-neighbor spatial networks', 'k = 15') +
  theme_void() +
  coord_quickmap() +
...
dat <- swsn %N>%
  mutate(ID = 1:n(),
         lon = pts$x, 
         lat = pts$y) %>%
  left_join(raster::extract(reof_raster, pts, df = TRUE), by = 'ID') %E>%
  ...
  mutate(name_from = .N()$name[from], 
         name_to = .N()$name[to]) %>%
  left_join(distances_direct) %>%
  left_join(distances_indirect) %>%
  select(-c(name_from:name_to)) %>%
  mutate(
