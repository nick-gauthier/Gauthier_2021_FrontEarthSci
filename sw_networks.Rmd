...
dat <- swsn %N>%
  mutate(ID = 1:n(),
         lon = pts$x, 
         lat = pts$y) %>%
  left_join(raster::extract(env_rasters, pts, df = T), by = 'ID') %E>%
...
geo_models <- c(
  ...
  mutate(formula = map(formula_chr, as.formula), 
         cloglog = map(formula, fit_gam_cloglog),
         logit = map(formula, fit_gam_logit)) %>%
  gather(link, model, 3:4) %>%
  mutate(aic = map_dbl(model, AIC)) %>%
  arrange(aic)

geo_models[[1,4]] %>% plot(residuals = F)  
geo_models[[1,4]] %>% gam.check  
map(geo_models$model, summary)
map(geo_models$model, plot, scheme = 1, residuals = F)
map(geo_models$model, plot, scheme = 1, residuals = T)
