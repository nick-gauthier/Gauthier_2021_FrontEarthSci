...
plot_swsn <- function(net, ncols = 3){
  ...
  scale_edge_color_viridis(name = 'Similarity', option = 'magma') +#, limits = c(0,1)
    #scale_edge_color_distiller(palette = 'Spectral', guide = 'legend', name = 'Similarity', limits = c(0,1)) +
  coord_sf(datum = NA) +
  theme_void() +
...
plot(geo_models$gam[[1]], trans = plogis)
```
...
plt_dat <- swsn_reof %E>% 
  ...
  mutate(similarity = if_else(similarity > .999, .999, similarity),
         ...
         pred_null = plogis(predict(geo_models$gam[[3]], newdata = as_tibble(.), type = 'response')),
         res = similarity - pred_null)
...
arrange(-res) %>%
ggraph('manual', x = pts$x, y = pts$y) +
  geom_edge_fan(aes(alpha = -1 * res, color = res)) +
  scale_edge_alpha(guide = 'none') +
    scale_edge_color_viridis(option = 'magma', guide = 'none',limits = c(0,1)) +
  coord_sf(datum = NA) +
  labs(subtitle = 'D') +
...
```{r}
m1 <- dat %>%
  filter(tie == TRUE, time == 1300) %>%
  gamm(qlogis(similarity) ~ s(distance, bs = 'cr', k = 5) + s(eof1, bs = 'cr', k = 5)+s(eof2, bs = 'cr', k = 5)+s(eof3, bs = 'cr', k = 5)+s(eof4, bs = 'cr', k = 5), method = 'ML', 
      family = gaussian(),
      correlation = corMLPE(form = ~from + to),
      data = .)
