...
```{r distance_matrix, cache = TRUE, dependson=c('conductance')}
dist_mat <- pts %>% # site locations
  as.matrix %>%
...
nn_mat <- dist_mat %>% 
  ...
  purrr::map(as_tbl_graph) %>% # convert back to tidygraph objects
...
```{r}
paths_sf <- path_dat %>% 
  mutate(paths = purrr::map(paths, st_as_sf)) %>% 
  pull(paths) %>% 
  do.call(rbind, .) %>%
...
```{r fig.fullwidth = TRUE, fig.width = 10, echp = FALSE}
paths_sf %>%
  inner_join(activate(swsn, 'edges') %>% as_tibble, .) %>%
  st_sf %>%
  as_Spatial %>%
...
ggplot(aes(long, lat)) +
  ...
  geom_path(aes(group = group), alpha = .2) +
...
eigs <- prcomp(t(as.data.frame(spei_obs %>% crop(bbox), na.rm = TRUE)), scale. = FALSE)[["sdev"]]^2
...
obs_eof <- as.matrix(eof)
colnames(obs_eof) <- colnames(amp) <- paste("EOF", 1:4, sep = "")
rownames(obs_eof) <- colnames(t(as.data.frame(spei_obs, na.rm = T)))
...
amp <- scale(scores)
attributes(amp)$`scaled:center` <- attributes(amp)$`scaled:scale` <- NULL
recon_eof <- as.matrix(eof)
colnames(recon_eof) <- colnames(amp) <- paste("EOF", 1:4, sep = "")
rownames(recon_eof) <- colnames(t(as.data.frame(spei_recon, na.rm = T)))
```
...
```{r plot_rrecon_eofs, fig.margin = TRUE, echo = FALSE, fig.cap = 'Reconstructed REOFs'}
...
```{r plot_recon_eofs, fig.margin = TRUE, echo = FALSE, fig.cap = 'Reconstructed REOFs'}
as.data.frame(spei_recon[[1]], xy = TRUE, na.rm = TRUE) %>% cbind(recon_eof) %>%
  select(-3) %>%
...
```{r plot_obs_eof, echo = FALSE, fig.width = 6, fig.height = 4, fig.cap = 'Observed drought REOFs'}
...
```{r plot_robs_eof, echo = FALSE, fig.width = 6, fig.height = 4, fig.cap = 'Observed drought REOFs'}
as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>% 
  cbind(obs_reof$REOF) %>%
  select(-3) %>%
  mutate(EOF1 = -1 * EOF1) %>%
...
```{r}
plot(reof_raster%>% crop(bbox))
brick(c(reof_raster, env_rasters[[9:12]])) %>% crop(bbox) %>% as.data.frame(xy = T, na.rm = T) %>% gather(reof, value, 3:10) %>% ggplot(aes(x, y)) + geom_raster(aes(fill = value)) +facet_wrap(~reof, nrow = 4) + coord_quickmap() +   scale_fill_distiller(palette = 'BrBG', direction = -1, limits = c(-1,1)) 
pairs(env_rasters[[c(1:4,9:12)]] %>% crop(bbox)) 
pairs(env_rasters[[1:8]] %>% crop(bbox)) 
brick(c(reof_raster, env_rasters[[9:12]])) %>% crop(bbox) %>% pairs
plot(env_rasters[[1:4]])
cor(env_rasters[[1]], env_rasters[[2]])
dat %>%select(reof1:reof4, pc1:pc4) %>% pairs
```
```{r}
as.data.frame(env_rasters[[1:4]], xy = TRUE, na.rm = TRUE) %>% 
 # mutate(EOF1 = -1 * EOF1) %>%
  gather(pc, value, 3:(n_modes + 2)) %>%
  #mutate(value = value * -1) %>%
  group_by(pc) %>%
...
similarity_models[[3,5]]$gam %>% glance

t2 <- t1[c(1,4,8,11,14),] %>%
  select(time, data, model) %>%
  mutate(model = map(model, ~.$gam),
...
t2 %>%
  ...
  ggplot(aes(pc4, pred, group = time, color = time)) +
  geom_point()
  geom_smooth()
```
...
t1[[1,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,6))
t1[[4,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,6))
t1[[8,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,6))
t1[[11,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,6))
t1[[14,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,6))


t1[[1,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,4))
t1[[4,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,4))
t1[[8,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,4))
t1[[11,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,4))
t1[[14,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,4))

t1[[1,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,5))
t1[[4,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,5))
t1[[8,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,5))
t1[[11,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,5))
t1[[14,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,5))

t1[[1,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,2))
t1[[4,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,2))
t1[[8,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,2))
t1[[11,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,2))
t1[[14,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,2))
```

```{r}
t1[[2,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[5,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[7,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[10,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[13,5]]$gam %>% getViz %>% sm(2) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))

t1[[2,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[5,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[7,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[10,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[13,5]]$gam %>% getViz %>% sm(3) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))

t1[[2,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[5,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[7,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[10,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))
t1[[13,5]]$gam %>% getViz %>% sm(4) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.4))

t1[[2,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[5,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[7,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[10,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2)+ scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
t1[[13,5]]$gam %>% getViz %>% sm(5) %>% plot(allTerms = T, trans = plogis) + l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) + scale_y_continuous(limits = c(0,1))+ scale_x_continuous(limits = c(0,.6))
```
```{r}
t1[[10,5]]$gam %>% getViz %>% plot(allTerms = T) %>% print(pages=1)
t1[[13,5]]$gam %>% getViz %>% plot(allTerms = T, trans = plogis) %>% print(pages=1)
t1[[7,5]]$gam %>% getViz %>% plot(allTerms = T, trans = plogis) %>% print(pages=1)
t1[[5,5]]$gam %>% getViz %>% plot(allTerms = T, trans = plogis) %>% print(pages=1)
t1[[2,5]]$gam %>% getViz %>% plot(allTerms = T, trans = plogis) %>% print(pages=1)
