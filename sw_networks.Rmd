...
A key assumption here is that the REOFs calculated from bot the observations and reconstructions correspond to the same physical phenomena. That way we can just use the observed patterns as the high resolution patterns, and infer their temporal evolution from the reconstructions. This means we don't have to downscale the reconstructed reofs to use them, as otherwise we'd have to deal with issues of spatial represnetaitveness an non-overlap. Now we can be ensured that these are the same signals.
...
```{r}
#gam.check(mod1)
mod1_viz <- getViz(geo_models[[1,4]] )
