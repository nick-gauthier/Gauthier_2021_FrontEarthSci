...
ggplot(aes(x = rank, y = eig_pct)) +
  ...
  theme_bw() + scale_y_continuous(trans = 'log')
...
reof_dat<-  left_join(recon_reof$amplitude %>%
  ...
  obs_reof$amplitude %>%
    as_tibble %>%
    mutate(year = 1896:1999), by = 'year')

t1 <- gam(EOF1.y ~ s(EOF1.x) + s(EOF2.x) + s(EOF3.x) + s(EOF4.x) + s(EOF5.x) + s(EOF6.x),
    method = 'REML', select = T, data = reof_dat)

summary(t1)
plot(t1, residuals = T)

paste0('s(',1:10, ')') %>% paste(collapse = ' + ') %>% paste('reof_dat$EOF1.y', ., sep = ' ~ ') %>% as.formula %>% gam(method = 'REML', select = T, data = test_dat)
```
