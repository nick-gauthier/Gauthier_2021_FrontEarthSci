...
load('geo_models')
geo_models
summary(geo_models$gam[[1]])

#concurvity(geo_models$gam[[1]])
#concurvity(geo_models$gam[[2]])
```

```{r}
evaluate_smooth(geo_models$gam[[1]], 's(distance)')  %>% draw()
evaluate_smooth(geo_models$gam[[1]], 's(distance)') %>% 
...
geo_models$gam[[1]] %>% getViz %>% sm(1) %>%  plot(trans = plogis) +geom_vline(xintercept = c(8, 80), linetype = 2)+ l_fitLine(colour = "red") + l_ciLine(mul = 5, colour = "blue", linetype = 2) 
ggsave('figures/distance_function.pdf', width = 6, height = 4)
```
```{r}
draw(geo_models$gam[[1]], scales = 'fixed')
...
swsn_reof %E>% 
  filter(similarity > 0) %>%
  mutate(pred = predict(geo_models$gam[[3]], newdata = as_tibble(.), type = 'response')) %>%
    mutate(res = qlogis(similarity) - (pred)) %>%
  as_tibble %>%
  ggplot(aes(eof2, res, group = time, color = as.factor(time))) +
  geom_smooth()
```
...
plt_dat <- swsn_reof %E>% 
  filter(time == 1200, 
         similarity > 0) %>%
  mutate(similarity = if_else(similarity > .999, .999, similarity),
         pred_mod = plogis(predict(geo_models$gam[[1]], newdata = as_tibble(.), type = 'response')),
         pred_null = plogis(predict(geo_models$gam[[5]], newdata = as_tibble(.), type = 'response')),
...
```{r}
## Plot out different eof networks
swsn_reof %E>% 
  filter(time == 1200, 
         similarity > 0) %>%
arrange(log(eof6)) %>%
ggraph('manual', x = pts$x, y = pts$y) +
  geom_edge_fan(aes(alpha = eof6, color = eof6)) +
  scale_edge_alpha(guide = 'none', limits = c(0,1)) +
  scale_edge_color_viridis(option = 'magma', guide = 'none') +
  coord_sf(datum = NA) +
   labs(subtitle = 'A') +
...
swsn_reof %E>% 
  filter(time == 1200, 
         similarity > 0) %>%
  mutate(eof3 = log(eof3+.001)) %>%
arrange(eof3) %>%
ggraph('manual', x = pts$x, y = pts$y) +
  geom_edge_fan(aes(alpha = eof3, color = eof3)) +
  scale_edge_alpha(guide = 'none', limits = c(0,1)) +
  scale_edge_color_viridis(option = 'magma', guide = 'none') +
  coord_sf(datum = NA) +
   labs(subtitle = 'A') +
...
m1 <- dat %>%
  filter(tie == TRUE) %>%
  filter(time == 1200) %>%
  gam(similarity ~ s(from, bs = 're') + s(to, bs = 're'), data = ., method = 'ML', family = betar)
plot(m1)
gam.check(m1)
summary(m1)
summary(m2)
m2 <- dat %>%
  filter(tie == TRUE) %>%
  bam(similarity ~ s(distance, k = 5) + s(from, bs = 're', by = time) + time, data = ., select = TRUE, discrete = FALSE, family = betar)
m3 <- dat %>%
  filter(tie == TRUE) %>%
  bam(similarity ~ s(distance, k = 5) + s(to, bs = 're', by = time) + time, data = ., select = TRUE, discrete = FALSE, family = betar)
m4 <- dat %>%
  filter(tie == TRUE) %>%
  bam(similarity ~ s(distance, k = 5) + s(size, k = 5, by = time) + time, data = ., select = TRUE, discrete = FALSE, family = betar)
AIC(m1,m2,m3,m4)
swsn_reof %E>% 
  filter(similarity > 0) %>%
...
swsn_reof %E>% 
  filter(similarity > 0) %>%
  mutate(pred = predict(m3, newdata = as_tibble(.), type = 'response')) %>%
  arrange(pred) %>%
ggraph('manual', node.positions = pts) +
  geom_edge_fan(aes(alpha = pred, color = pred)) +
  geom_sf(data = states, fill = NA, color = 'black') +
