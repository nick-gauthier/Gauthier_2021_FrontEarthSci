...
```{r height_difference, cache = TRUE}
altDiff <- function(x){x[2] - x[1]}
hd <- transition(elev, altDiff, 8, symm = FALSE)
```

```{r, echo = FALSE}
# these routines can be memory intensive, so periodically free up some space
gc()
```
...
```{r}
cell.targets <- Which(!is.na(aggregate(elev, fact = 2)), cells = TRUE) # can't go to water cells
adj <- adjacent(aggregate(elev, fact = 2), cells = cell.targets, target = cell.targets, pairs = TRUE, directions = 8)
```
...
speed_r <- slope_r
speed_r[adj] <- 6 * exp(-3.5 * abs(slope_r[adj] + 0.05))
conductance_r <- geoCorrection(speed_r, type = 'r')
```


```{r echo = FALSE, eval=FALSE}
conductance_c <- readRDS('conductance.RDS')
conductance_c %>%
  raster %>%
...
```{r eval = FALSE}
pas <- passage(conductance_r, as.matrix(pts), as.matrix(pts), theta = .05)
pts
commute_dist <- pts %>%
  as.matrix %>%
...
```{r eval = FALSE}
saveRDS(dist_mat, 'distance_matrix.rds')
```
...
```{r}
dist_mat <- readRDS('distance_matrix.rds')
time_mat <- dist_mat %>%
  as_tbl_graph(directed = FALSE) %E>%
  rename(distance = weight) %>%
  filter(distance <= 3600 * 5)

ggraph(time_mat_6, 'manual', node.positions = pts) +
...
swsn %E>%
  ...
  right_join(fortify(SpatialLinesDataFrame(merged.path, data = data.frame(id = 1:13264)))) %>%
  ggplot(aes(long, lat, group = group)) +
  geom_path(alpha = .1, aes (color = similarity)) +
