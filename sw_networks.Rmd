...
## Data sources
This R Markdown script requires raster data of [observed present-day temperature and precipitation](https://vdeblauwe.wordpress.com/download/)
...
```{r plot_eof_obs, echo = FALSE, fig.cap = 'Observed drought REOFs'}
...
```{r plot_amplitudes, echo = FALSE, fig.cap = 'Amplitude reconstruction'}
...
recon_amplitudes <- amplitudes %>%
  group_by(eof) %>%
  nest %>%
  mutate(mod = purrr::map(data, ~gam(obs ~ s(recon, bs = 'cr'), data = ., method = 'REML', select = TRUE))) %>%
  left_join(past_amplitudes, by = 'eof') %>%
  mutate(predictions = purrr::map2(mod, recons, ~predict(.x, .y, type = 'response')),
         predictions = purrr::map(predictions, ~.[1:400]),
         predictions = purrr::map(predictions, ~tibble(year = 1100:1499, amplitude = .)),
         predictions = purrr::map(predictions, ~mutate(., amp_smooth = zoo::rollmean(amplitude, k = 51, fill = NA)))) %>%
...
```{r plot_recon_amplitudes, echo = FALSE, fig.margin = TRUE}
...
## Network data
Now we move on to the archaeological social network proxies. First we read in a csv file containing information for all of the archaeological sites in the database, including spatial location and number of rooms present from each time period. These will become the node-level data for the network.
```{r}
...
env_rasters <- list(elev, prec, temp, reof_raster) %>%
  purrr::map(projectRaster, prec) %>%
  brick %>%
  `names<-`(c('elev', 'precip', 'temp', 'reof1', 'reof2', 'reof3', 'reof4', 'reof5'))
```
...
dat <- swsn %N>%
  ...
  mutate(size_from = case_when(time == 1200 ~ .N()$P1room[from],
         ...
         elev_from = .N()$elev[from],
         elev_to = .N()$elev[to],
         precip_from = .N()$precip[from],
         precip_to = .N()$precip[to],
         temp_from = .N()$temp[from],
         temp_to = .N()$temp[to],
...
In log log space, we get the downward function from devries et al 2009!
```{r}
...
fit_gam_logit <- function(x){
  bam(x, method = 'REML', select = TRUE, 
         family = betar(link = 'logit', eps = .001),#1/(2*nrow(dat))), # MacMillan and Creelman 2005 pp 8-9
         data = dat)#(dat * (nrow(dat) - 1) + .5) / nrow(dat))
}
...
```{r geo_models_output}
#geo_models[[1,4]] %>% plot(residuals = FALSE)  
#geo_models[[1,4]] %>% gam.check  
#purrr::map(geo_models$model, summary)
#purrr::map(geo_models$model, plot, scheme = 1, residuals = FALSE)
#purrr::map(geo_models$model, plot, scheme = 1, residuals = TRUE)
#gam.check(geo_models[[5,2]])
```
