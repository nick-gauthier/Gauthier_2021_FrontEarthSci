...
spei_obs <- ((brick('data/spei12_6_PRISM.nc') + 
  ...
  crop(bbox) %>% # crop to the study area bounding box
...
eigs <- prcomp(t(as.data.frame(area_weight(spei_obs), na.rm = TRUE)), scale. = FALSE)[["sdev"]]^2
eigs.pct <- 100 * eigs/sum(eigs)
eigs.lo <- eigs * (1 - sqrt(2/n_effective(spei_obs)))
eigs.hi <- eigs * (1 + sqrt(2/n_effective(spei_obs)))
...
spei_recon %>%
  ...
  as.data.frame(na.rm = TRUE) %>%
  t %>%
  eofNum(n = n_effective(spei_recon), scale. = FALSE)
```
...
reof_raster <- as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>%
  ...
  projectRaster(bio_clim %>% crop(bbox))

plot(which.max(abs(reof_raster)))
plot(reof_raster)
```
...
```{r plot_robs_eof, echo = FALSE, fig.width = 6, fig.height = 6, fig.cap = 'Observed drought REOFs'}
as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>% 
  cbind(obs_reof$REOF) %>%
  select(-3) %>%
  mutate(EOF1 = -1 * EOF1, EOF4 = -1 * EOF4) %>%
  gather(eof, value, 3:(n_modes + 2)) %>%
  ...
  facet_wrap(~eof) +
  theme_void() +
  coord_quickmap(xlim = c(-115, -103), ylim = c(31, 38)) +
...
recon_amplitudes <- amplitudes %>%
  ...
  mutate(mod = purrr::map(data, ~gam(obs ~ s(recon, bs = 'cr'), 
                                     data = ., method = 'REML', 
                                     select = TRUE))) %>%
  left_join(past_amplitudes, by = 'reof') %>%
...
```{r cache = TRUE}
clim_pca <- RStoolbox::rasterPCA(bio_clim %>% crop(bbox), spca = TRUE, nComp = 4)
```
...
plot(clim_pca$map)
plot(which.max(abs(clim_pca$map)))
```
