...
ggplot(data = d, aes(x = rank, y = eigs)) + geom_errorbar(aes(x = rank, 
        ...
        theme(panel.grid.minor = element_blank()) + scale_y_continuous(trans = 'log')

eigs <- prcomp(t(as.data.frame(area_weight(spei_recon), na.rm = TRUE)), scale. = FALSE)[["sdev"]]^2
eigs.pct <- 100 * eigs/sum(eigs)
eigs.lo <- eigs * (1 - sqrt(2/n_effective(spei_recon)))
eigs.hi <- eigs * (1 + sqrt(2/n_effective(spei_recon)))
cumvar <- round(cumsum(eigs.pct), 1)
p <- 62
d <- data.frame(rank = factor(1:p), eigs, eigs.lo, eigs.hi, 
        cumvar)
...
t1 <- gam(EOF2.y ~ s(EOF1.x) + s(EOF2.x) + s(EOF3.x) + s(EOF4.x) + s(EOF5.x) + s(EOF6.x),
