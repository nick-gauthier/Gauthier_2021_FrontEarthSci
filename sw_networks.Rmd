...
```{r swsn_import}
swsn <- list.files('data/wares/', full.names = TRUE) %>%
  imap(read_swsn) %>%
...
sim_tmp <- read.csv('data/wares/AD1200js', row.names = 1, check.names = FALSE) %>%
  as.matrix %>%
  .[1:3,1:3]
knitr::kable(sim_tmp, caption = 'Example JSD matrix')
```
...
plot_swsn(swsn, ncols = 3)
ggsave('figures/JSD_network.png', width = 7)
```
...
reof_raster %>%
  ...
  scale_fill_distiller(palette = 'BrBG', limits = c(-1, 1)) +
  geom_sf(data = states, fill = NA, color = 'black') +
  geom_sf(data = filter(sites, P4room > 0)) +
  facet_wrap(~layer) +
  coord_sf(datum = NA) +
  theme_void()
```
...
cells <- reof_raster %>%
  aggregate(fact = 2) %>%
  raster::extract(sites, cellnumbers = TRUE) %>%
  .[,1] %>%
  as.character

sites <- mutate(sites, cell = cells)

load('ware_matrices')

library(philentropy)
getJS <- function(dat, period){
  names <- pull(dat, cell)
  dat %>%
    .[,-1] %>%
  select_if(~!all(. == 0)) %>%
  ...
  `rownames<-`(names) %>%
    `colnames<-`(names) %>%
    as_tbl_graph %E>%
        mutate(time = 1150 + 50 * period)  # set the time period

}

coarse_jsd <-left_join(list(AD1200cer, 
          AD1250cer, 
          AD1300cer, 
          AD1350cer, 
          AD1400cer) %>%
  purrr::map(rownames_to_column) %>%
  bind_rows(.id = 'period_num') %>%
  mutate(period_num = as.numeric(period_num)) %>%
  rename(SWSN_Site = rowname),
sites %>%
  gather(period, rooms, P1room:P6room) %>%
  mutate(period_num = parse_number(period)) %>%
  filter(period_num < 6)) %>%
  group_by(period_num, cell) %>% 
  select(`Babacomari Series`:`Trincheras Series`) %>%
 summarise_all(sum, na.rm = TRUE) %>%
  nest %>%  
  mutate(net = map2(data, period_num, getJS)) %>%
  pull(net) %>%
  reduce(graph_join, by = 'name') %>%
  filter(!edge_is_loop()) %>%
  rename(JSD = weight) %>%
...
coarse_jsd %E>%
    ...
    filter(from < to) %>%
     mutate(JSD = qlogis(if_else(JSD > .999, .999, JSD))) %>%
    arrange(JSD) %>%
ggraph('manual',node.positions = pts_jsd) +
  geom_edge_fan(aes( color = JSD)) +
          geom_sf(data = states, fill = NA, color = 'black') +
...
swsn_reof <- swsn %N>%
  ...
  convert(to_undirected) %>%
    mutate(dist_geo = geosphere::distGeo(st_coordinates(.N()$geometry[from]), st_coordinates(.N()$geometry[to])) / 1000) %N>%
...
m1 <- dat %>%
  filter(tie == TRUE) %>%
  gam(JSD ~ s(from, bs = 're') + s(to, bs = 're') + s(distance, bs ='cr', k = 5) + time, data = ., discrete = TRUE, family = betar)
m2 <- dat %>%
  filter(tie == TRUE) %>%
  gam(JSD ~ s(from, bs = 're') + s(distance, bs ='cr', k = 5) + time, data = ., discrete = TRUE, family = betar)

m3 <- dat %>%
  filter(tie == TRUE) %>%
  gam(JSD ~ s(from, bs = 're') + s(to, bs = 're') + s(distance, bs ='cr') +
       s(reof1, time, bs = 'fs') + s(reof2, time, bs = 'fs') + 
        s(reof3, time, bs = 'fs') + s(reof4, time, bs = 'fs') +
        s(reof5, time, bs = 'fs') + s(reof6, time, bs = 'fs'), data = .,discrete = TRUE, family = betar)
m4 <- dat %>%
  filter(tie == TRUE) %>%
  gam(JSD ~ s(from, bs = 're') + s(distance, bs ='cr', k = 5) +
        s(eof1, bs = 'cs', k = 6) +
       s(eof2, bs = 'cs', k = 6) +
       s(eof3, bs = 'cs', k = 6) +
         s(eof4, bs = 'cs', k = 6) +
         s(eof5, bs = 'cs', k = 6) +
         s(eof6, bs = 'cs', k = 6) + time, data = ., discrete = TRUE, family = betar)
