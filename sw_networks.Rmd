...
read_swsn <- function(net, time){
  read.csv(net, row.names = 1, check.names = FALSE) %>% # import the similarity matrix
...
```{r nearest_neighbor}
nn_mat <- dist_mat %>% # start with the distance matrix
  ...
  morph(to_split, time, split_by = 'edges') %>% # split the graph into subgraphs by time period
  group_by(from) %>% # group by each edge's from node
  ...
  map(as.undirected, mode = 'collapse', edge.attr.comb = 'first') %>% # turn into undirected again
...
ggplot(dat, aes(distance, similarity)) +
  ...
  theme_minimal()
```
```{r, logistic-model, include = FALSE}
m1 <-  bam(tie ~ s(distance, bs = 'tp', m = 2) + s(distance, by = time, bs = 'ts', m = 1) + s(time, bs = 're', k = 5), discrete = T,
         family = binomial,
         data = dat)
summary(m1)
plot(m1)
...
fit_gam <- function(x){
  gam(x, method = 'ML', 
         ...
         control = ctrl,
         eps = .0001,
         data = dat)#filter(dat, similarity > 0 & similarity < 1))#(dat * (nrow(dat) - 1) + .5) / nrow(dat))
}
```
...
```
```{r}
expand.grid(distance = seq(0, 100000, by = 100), time = as.factor(c(1200, 1250, 1300, 1350, 1400))) %>%
  mutate(., pred = predict(geo_models[[1,3]], ., type = 'response')) %>%
  ggplot(aes(distance, pred, group = time, color = time)) +
  geom_line()
```
...
```{r}
dat %>%
  mutate(resid = residuals(geo_models[[1,3]], type = 'response')) %>%
ggplot(aes(size_from * size_to, qlogis(resid))) +
