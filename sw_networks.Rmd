...
Zoom in on the study area.
```{r plot_amplitudes, fig.fullwidth=TRUE, echo = FALSE, fig.cap = 'Strong linear correlation between the observed and reconstructed EOFs'}
...
We can fit gams between the observed and reconstrcuted PC time series, and then use these to predict the eof amplitudes back in tmie. Adding in this regression step essentalially bias-corrects the reconstructed amplitudes
```{r recon_amplitudes}
...
```{r plot_recon_amplitudes, echo = FALSE}
...
xtable(head(node_data[, 1:6]), caption = "First rows of node level data")
```
...
```
```{r, echo = FALSE}
xtable(head(dat[, 1:6]), caption = "First rows of node level data")
```
...
ggplot(dat, aes(distance, similarity)) +
  geom_point(alpha = .1) +
  geom_smooth() +
  ggtitle('Empirical distance decay function')
```


```{r, fig.margin = TRUE, fig.cap='In log log space, we get the downward function from devries et al 2009!'}
