...
slope <- seq(-1, 1, .01)
qplot(slope, tobler_adjusted(abs(slope)), geom = 'line' ) +
...
ggplot(aes(x = PC, y = percent * 100)) +
  ...
  geom_text(aes(x = PC, y = cumvar_line, label = round(cumulative, 1)), size = 3, vjust = 0) +
  labs(list(x = "Rank", y = "Normalized Eigenvalue")) + scale_x_continuous(limits = c(0, 15)) +
  theme_bw() #+scale_y_continuous(trans = 'log') + stat_smooth(aes(weight = weights), method = 'lm') 
...
eofs_obs <- obs_pca %>%
  ...
  group_by(PC) %>%
  nest %>%
  left_join(obs_eigs[1:2]) %>%
  mutate(data = map2(data, std.dev, ~mutate(.x, value = value * .y))) %>%
  unnest %>%
  bind_cols(as.data.frame(spei_obs[[1]], xy = T, na.rm = T)[1:2] %>% slice(rep(1:n(), times = 9)))

eofs_recon <-recon_pca %>%
  tidy(matrix = 'variables') %>%
  filter(PC <= 9) %>%
  group_by(PC) %>%
  nest %>%
  left_join(recon_eigs[1:2]) %>%
  mutate(data = map2(data, std.dev, ~mutate(.x, value = value * .y))) %>%
  unnest %>%
  bind_cols(as.data.frame(spei_recon[[1]], xy = T, na.rm = T)[1:2] %>% slice(rep(1:n(), times = 9)))
...
eof_data <- raster::extract(reof_raster, pts, df=TRUE) %>%
  gather(eof_num, value, 2:(n_modes_recon + 1)) %>%
  full_join(test) %>%
...
test
gc()
test1 <- gamm(qlogis(similarity) ~ s(distance,bs = 'cr', k = 6)   +
...
gam.check(test1$gam)
summary(test1$gam)
AIC(test2$lme, test6$lme)
plot(test6$gam, pages = 1, scale = 0)
plot(test1$gam, pages = 1)
```
```{r}
