...
plot_swsn <- function(net, ncols = 3){
  net %E>%
    filter(similarity > .001) %>%
    filter(from < to) %>%
     mutate(similarity = qlogis(if_else(similarity > .999, .999, similarity))) %>%
    arrange(similarity) %>%
  ggraph('manual', x = pts$x, y = pts$y) +
  geom_edge_fan(aes( color = similarity)) +
          geom_sf(data = states, fill = NA, color = 'black') +
  facet_edges(~time, ncol = ncols) +
  scale_edge_alpha(guide = 'none') +
  #scale_edge_color_viridis(name = 'Similarity', option = 'magma') +#, limits = c(0,1)
  scale_edge_color_distiller(palette = 'RdYlBu', guide = 'legend', name = 'Similarity') +
  coord_sf(datum = NA) +
  theme_void() +
...
ggsave('figures/similarity_network.png', width = 7)
```
...
swsn_reof <- swsn %N>%
  ...
  #filter(between(similarity, .001, .999)) %>%
...
#         trans = local_transitivity(weights = similarity),
#         cluster = group_leading_eigen(weights = similarity)) %E>%
...
ggplot(dat, aes(qlogis(similarity))) +
  geom_histogram() + facet_wrap(~time)
...
plt_dat <- swsn_reof %E>% 
  ...
  mutate(similarity = if_else(similarity > .999, .999, similarity),
         ...
         res_null = (similarity) - pred_null,
         res_mod = (similarity) - pred_mod)
...
ggraph('manual', x = pts$x, y = pts$y) +
  ...
  scale_edge_color_distiller(palette = 'RdYlBu', guide = 'none', name = 'Similarity', limits = c(0,1)) +
  coord_sf(datum = NA) +
   labs(subtitle = 'A') +
  theme_void()
...
ggraph('manual', x = pts$x, y = pts$y) +
  ...
  #scale_edge_color_viridis(option = 'magma', guide = 'none') +
    scale_edge_color_distiller(palette = 'RdYlBu', guide = 'none', name = 'Similarity', limits = c(-1,1)) +
  coord_sf(datum = NA) +
   labs(subtitle = 'C') +
  theme_void()
...
m1 <- dat %>%
  filter(tie == TRUE) %>%
  gam(similarity ~ s(from, bs = 're') + s(to, bs = 're') + s(distance, bs ='cr', k = 5) + time, data = ., discrete = TRUE, family = betar)
m2 <- dat %>%
  filter(tie == TRUE) %>%
  gam(similarity ~ s(from, bs = 're') + s(distance, bs ='cr', k = 5) + time, data = ., discrete = TRUE, family = betar)

m3 <- dat %>%
  filter(tie == TRUE) %>%
  gam(similarity ~ s(from, bs = 're') + s(to, bs = 're') + s(distance, bs ='cr') +
...
m4 <- dat %>%
  filter(tie == TRUE) %>%
  gam(similarity ~ s(from, bs = 're') + s(distance, bs ='cr', k = 5) +
        s(eof1, bs = 'cs', k = 6) +
       s(eof2, bs = 'cs', k = 6) +
       s(eof3, bs = 'cs', k = 6) +
         s(eof4, bs = 'cs', k = 6) +
...
test1 <- gamm(qlogis(similarity) ~ s(distance, bs = 'cr') + time,
              method = 'REML',
