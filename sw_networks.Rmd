...
# Introduction
This document contains all analysis code used to produce the results and figures in the main text.
...
Import the reconstructed SPEI fields from PHYDA.
...
```{r plot_variance_trend, echo = FALSE, fig.margin=TRUE, fig.cap = 'Increasing variance of the reconstruction.'}
spei_recon %>% 
...
as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>% cbind(obs_eof$REOF) %>%
  select(-3) %>%
...
```{r plot_eof_recons, echo = FALSE, fig.cap = 'Reconstructed REOFs'}
...
ggraph(swsn, 'manual', node.positions = pts) +
  geom_raster(data = gather(as.data.frame(reof_raster, xy = TRUE, na.rm = TRUE), key, value,  3:6), aes(x, y, fill = value)) +
