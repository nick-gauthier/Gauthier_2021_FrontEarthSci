...
matrix(raster::extract(elev,raster::extent(elev),buffer=1000),
  ...
  sphere_shade(texture = "imhof1", zscale = 20, progbar = FALSE) %>%
  plot_map()
...
clusterEvalQ(cl, {library(yenpathy)})
paths <- mclapply(1:nrow(sim_net), function(x){
...
k_shortest_paths(spatial_net, 4, 3, k = 5)
...
test <- nn_mat %E>%
 ...
 shortest.paths(test, mode = 'all', weights =   test %E>% as_tibble %>% .$distance)

 all_shortest_paths(test, 1, mode = 'all', weights =   test %E>% as_tibble %>% .$distance)
  distances(test, 1, 16, mode = 'all', weights =   test %E>% as_tibble %>% .$distance)
 t1
 all_shortest_paths;shortest_paths
test %E>% edge
 
test %E>% morph(to_shortest_path, 1,5, mode = 'all', weights = distance) %>% mutate(test = 34) %>% unmorph



t2 <- nn_mat %E>% filter(time == 1300) %N>% filter(!node_is_isolated())


swsn %N>% mutate(lon = pts$x, lat = pts$y) %E>%
  mutate(distance1 = geosphere::distGeo(matrix(c(.N()$lon[from], .N()$lat[from]), ncol = 2), 
                            matrix(c(.N()$lon[to], .N()$lat[to]), ncol = 2)) / 1000) %>%
  mutate(distance2 = sqrt((.N()$EASTING[from] - .N()$EASTING[from])^2 + (.N()$NORTHING[from] - .N()$NORTHING[to])^2)) %>%
  left_join((dist_mat %>% as_tbl_graph(directed = T) %E>% filter(!edge_is_loop()) %>% as_tibble %>% rename(distance3 = weight)), by = c('from', 'to')) %>% 
  filter(time ==1300) %>% 
 # filter(from > to) %>% 
  mutate(distance4 = distances(t2, mode = 'all', weights =   t2 %E>% as_tibble %>% .$distance) %>% as_tbl_graph(directed = T) %E>% as_tibble %>% rename(distance4 = weight) %>% pull(distance4)) %>%
  as_tibble %>% select(distance1:distance4) %>% pairs

nn_mat %E>% filter(time == 1200)
```
...
ggraph(test_spat, 'manual', node.positions = pts) +
  geom_edge_link(alpha = .1, aes(color = sim)) +
  scale_edge_color_distiller(pal = 'Spectral')+
  #facet_edges(~time, nrow = 1) +
  theme_void() +
  coord_equal() +
  ggtitle('Nearest neighbor network', 'k = 15')
...
spei_obs %>% 
  ...
  geom_smooth() +
  ggtitle('Standardized Precipitation-Evapotranspiration Index in the American Southwest',
          'June-August, 12 month lag') + scale_x_continuous(limits = c(1896,2017)) +geom_smooth(method = 'lm') +
  theme_bw()
```
