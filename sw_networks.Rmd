...
states <- maps::map('state', regions = c('arizona', 'new mexico', 'colorado', 'california', 'utah', 'nevada'), fill = TRUE, plot = FALSE)
...
bbox_wus <- extent(c(-124, -102, 31, 42))
...
test <- spei_obs %>%
  ...
  as.data.frame(na.rm = TRUE) %>%
  t %>%
  prcomp(scale. = FALSE) %>% # work on the covariance matrix
  .[['sdev']] %>% # get the sd of the PCs
  `^`(2) %>% # square to get the eigenvalues 
  tibble(rank = 1:length(.), eigenvalues = .) %>%
  mutate(eig_pct = 100 * eigenvalues / sum(eigenvalues),
         error = sqrt(2 / n_effective(spei_obs)),
         low =  eigenvalues * (1 - error) * 100 / sum(eigenvalues),
         hi = eigenvalues * (1 + error) * 100 / sum(eigenvalues),
         cumvar = round(cumsum(eig_pct), 1),
...
test %>% 
  ...
  mutate(separated = if_else(is.na(lag(low)), TRUE, hi < lag(low)),
         ...
         weights = if_else(rank < 3, 0, 1))%>%
ggplot(aes(x = rank, y = eig_pct)) +
  geom_errorbar(aes(x = rank, ymin = low, ymax = hi), width = 0.4) +
  geom_point(size = 3, aes(color = as.factor(test))) + 
  geom_text(aes(x = rank, y = cumvar_line, label = cumvar), size = 3, vjust = 0) +
  labs(list(x = "Rank", y = "Normalized Eigenvalue")) + scale_y_continuous(trans = 'log') +
...
```{r}
gam(eigs ~ s(as.numeric(rank), k = 10, bs= 'ps', m = 0), data = d) %>% plot
```
...
obs_reof <- spei_obs %>% 
  ...
  eof(n_modes_obs, scale. = FALSE) # we don't rescale (ie we use the covariance matrix, because spei is already normalized and rescaled)

recon_reof <- spei_recon %>%
  area_weight %>%
  as.data.frame(na.rm = TRUE) %>%
  t %>% # transpose space and time
  eof(n_modes_recon, scale. = FALSE)
```
...
as.data.frame(spei_recon[[1]], xy = TRUE, na.rm = TRUE) %>% cbind(recon_reof$REOF) %>%
  select(-3) %>%
  gather(eof, value, 3:(n_modes_recon+2)) %>%
  ggplot(aes(x,y)) +
...
as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>% 
  ...
  gather(eof, value, 3:(n_modes_obs + 2)) %>%
  ...
  theme_void() +
    geom_polygon(data = states, aes(x = long, y = lat, group = region), color = 'black', fill = NA) +
  coord_quickmap() +
  ggtitle(paste('Leading', n_modes_obs, 'rotated empirical orthogonal functions'),
          'SPEI') +
  theme(legend.position = "bottom")
```
...
amplitudes <- left_join(
  recon_reof$amplitude %>%
    ...
    gather(reof, recon, 1:n_modes_recon),
  obs_reof$amplitude %>%
    as_tibble %>%
    mutate(year = 1896:1999) %>%
    gather(reof, obs, 1:n_modes_obs))
```
...
```{r}
reof_dat <- left_join(recon_reof$amplitude %>%
    .[797:900,] %>%
    as_tibble %>%
    mutate(year = 1896:1999),
  obs_reof$amplitude %>%
    as_tibble %>%
    mutate(year = 1896:1999), by = 'year', suffix = c('_recon','_obs'))%>%
  gather(eof_num, amplitude, (n_modes_recon+2):(n_modes_recon*2+1)) %>% 
  mutate(eof_num = as.factor(eof_num)) %>%
  mutate(new_eof = eof_num != lag(eof_num))

t1 <- paste0('s(EOF',1:9, '_recon, by = eof_num, k = 4)') %>% 
  paste(collapse = ' + ') %>% 
  paste('amplitude', ., sep = ' ~ ') %>% 
  as.formula %>% 
  bam(method = 'fREML', select = T, data = reof_dat, AR.start = new_eof)
summary(t1)
plot(t1)
...
recon_reof$amplitude %>%
  ...
  ggplot(aes(year, preds, group = eof_num)) +
...
```{r}
screeplot(clim_pca2$model, npcs = 20, type = 'lines')
lines(y = 1)
bio_clim %>%
  as.data.frame(na.rm = TRUE) %>%
  eofNum(scale. = TRUE)
...
reof_raster <- as.data.frame(spei_obs[[1]], xy = TRUE, na.rm = TRUE) %>%
  ...
  `names<-`(paste0('reof', 1:n_modes_obs)) %>%
...
```{r data_preparation}
rel_diff <- function(x, y)  pmax(x, y) / pmin(x, y)
dat <- swsn %N>%
  mutate(ID = 1:n(),
         lon = pts$x, 
         lat = pts$y) %>%
  ...
  # pc4 = abs(.N()$pc4[from] - .N()$pc4[to]),
   reof1 = rel_diff(.N()$reof1[from], .N()$reof1[to]),
   reof2 = rel_diff(.N()$reof2[from], .N()$reof2[to]),
   reof3 = rel_diff(.N()$reof3[from], .N()$reof3[to]),
   reof4 = rel_diff(.N()$reof4[from], .N()$reof4[to]),
    reof5 = rel_diff(.N()$reof5[from], .N()$reof5[to]),
   reof6 = rel_diff(.N()$reof6[from], .N()$reof6[to]),
   reof7 = rel_diff(.N()$reof7[from], .N()$reof7[to]),
   reof8 = rel_diff(.N()$reof8[from], .N()$reof8[to]),
  reof9 = rel_diff(.N()$reof9[from], .N()$reof9[to])) %>%
  as_tibble %>%
  group_by(time) %>%
  filter(from < to) %>%
  ungroup %>%
...
test <- dat %>%
  ...
  #filter(time %in% c(1200,1250,1300)) %>%
  #group_by(time) %>%
  sample_frac(.5) %>%
  mutate(time_num = as.numeric(time))

test
test1 <- gamm(qlogis(similarity) ~  s(distance, bs = 'tp', m = 2) + s(distance, by = time, bs = 'ts', m = 1, k = 4) + 
  s(pc1, bs = 'tp', m = 2) + s(pc1, by = time, bs = 'ts', m = 1) + 
  s(pc2, bs = 'tp', m = 2) + s(pc2, by = time, bs = 'ts', m = 1) +
  s(pc3, bs = 'tp', m = 2) + s(pc3, by = time, bs = 'ts', m = 1) + 
  s(pc4, bs = 'tp', m = 2) + s(pc4, by = time, bs = 'ts', m = 1) +
  time, method = 'REML', 
      family = gaussian(),
      correlation = corMLPE(form = ~from + to|time),
      data = test)

test2 <- gamm(qlogis(similarity) ~  te(distance, time_num) +
  te(pc1, time_num) +
  te(pc2, time_num) +
  te(pc3, time_num) +
  te(pc4, time_num),
...
test3 <- gamm(qlogis(similarity) ~  te(distance, time) +
  te(pc1, time_num) +
  te(pc2, time_num) +
  te(pc3, time_num) +
  te(pc4, time_num),
...
plot(test1$gam, scheme = 1)
 test1$gam %>% getViz %>% plot %>% print(pages =1)
