...
swsn_reof <- swsn %N>%
  ...
  convert(to_undirected) %>%
    mutate(dist_geo = geosphere::distGeo(st_coordinates(.N()$geometry[from]), st_coordinates(.N()$geometry[to])) / 1000) %>%
...
geo_models <- c(
 ...
 #    "s(distance, bs = 'cr') + time + s(eof1, bs = 'cs') + s(eof2, bs = 'cs') + 
 #        s(eof3,  bs = 'cs') + s(eof4, bs = 'cs')",
 #     "s(distance, bs = 'cr') + time + s(reof1, bs = 'cs') + s(reof2, bs = 'cs') + 
 #        s(reof3,  bs = 'cs') + s(reof4, bs = 'cs')",
      "s(distance, bs = 'cr', k = 6) + s(size, bs = 'cr',k = 6) +
s(log(rooms), bs = 'cs', k = 6) +
           s(elev, bs = 'cr', k = 6) +
         s(reof1, bs = 'cs', k = 6) +
         s(reof2, bs = 'cs', k = 6) +
         s(reof3, bs = 'cs', k = 6) +
         s(reof4, bs = 'cs', k = 6) + time + macro_same + micro_same + to"
   ) %>%
  paste('similarity ~', .) %>%
  tibble(formula_chr = .) %>%
...
plot(geo_models$model[[1]], trans = plogis, residuals = TRUE)
```
...
plot(test_models$model[[1]], scheme = 1, trans = plogis)
