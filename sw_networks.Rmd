...
lans <- function(g){
  ...
  mutate(total_weight = sum(weight), 
         fractional_weight = weight / total_weight) %>%
  nest %>%
  mutate(fractionals = map(data, ~.$fractional_weight)) %>%
  unnest(data, .preserve = fractionals) %>%
  mutate(ecdf = map2(fractional_weight, fractionals, ~.x >= .y)) %>%
  mutate(test = map_dbl(ecdf,sum)) %>%
  mutate(outs =  map_dbl(ecdf, length)) %>%
  mutate(test2 = 1 - test / outs) %>%
  filter(test2 <= .05) %>%
  select(from, to, weight)
  
  g %E>%
    semi_join(g_filtered, by  = c('from', 'to')) %>%
  return
}
...
swsn <- swsn %N>%
  ...
  mutate(from_x = .N()$lon[from],
         ...
         size_to = case_when(time == 1 ~ .N()$P1room[to],
                             ...
                             time == 4 ~ .N()$P4room[to],
                             time == 5 ~ .N()$P5room[to])) %>%
  mutate(size = size_from * size_to)
```

```{r echo = F, eval = F}
#Alternative version of code for directed network
swsn <- swsn %N>%
  mutate(lon = pts$x, 
         lat = pts$y) %E>%
  mutate(from_x = .N()$lon[from],
...
mod3 <- gam(similarity ~ s(distance, bs = 'cr') +
                         s(size, bs = 'cr') +
                        ti(distance, size),
            method = 'REML',
            family = betar(link = 'cloglog'),
...
mod4 <- gam(similarity ~ te(distance, size),
            method = 'REML',
            family = betar(link = 'cloglog'),
            data = dat)

mod5 <- gam(similarity ~ s(distance, bs = 'cr') +
              s(eof1, bs = 'cr') +
              s(eof2, bs = 'cr') +
              s(eof3, bs = 'cr') +
              s(eof4, bs = 'cr'),
...
plot(mod1, residuals = T);plot(mod2, residuals = T);plot(mod3, scheme = 1, residuals = T);plot(mod4, scheme = 1);plot(mod5, residuals = F)
